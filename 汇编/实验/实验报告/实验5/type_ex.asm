...
str2 		db 	'christmas is a time of joy and love.' 
			db 	0dh,0ah,'$' 
str3 		db	'store windows hold toys and gifts.'
			db 	0dh,0ah,'$' 
...
first:		mov 		ah,0			;set video mode 
			...
			int 		21h 
			mov 		si,0 
next:		mov 		dx,saddr[si]	;display sentence 
			mov 		ah,09h 
			int 		21h 
...
;---------------------------------------------------------
disptime 	proc 		near
;subroutine to display typping time for min:sec:msec 
			mov 		ax,min 
...
;----------------------------------------------
decdiv 		proc 		near 
;sub-subroutine divide number in AX by CX 
